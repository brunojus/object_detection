...
from imutils.video import WebcamVideoStream,FPS,FileVideoStream
...
class ObjectDetection:
    def __init__(self, id): 
        self.cap = cv2.VideoCapture(id, cv2.CAP_FFMPEG)
        # self.cap = FileVideoStream('drone.mp4').start()
        # self.cap = FileVideoStream(src = id).start()
    ...
    def main(self):
        ...
        #            im_dim = im_dim.repeat(output.size(0), 1)
                    ...
                    cv2.imshow("Object Detection Winames['Current Object'])+ ' ' +'is'+' at {} ".format(round(distance))+"cm")
